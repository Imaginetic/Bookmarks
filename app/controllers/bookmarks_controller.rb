class BookmarksController < ApplicationController
  ...
  def getBookmarksByCategory
    ...
    c_name = current_category.name

    json_categories = { "bookmarks" => []}

    
    cat_parent= { c_name => []}
    json_categories["bookmarks"].push( cat_parent)


    if current_category.bookmarks.count > 0
      json_categories["bookmarks"][0][c_name].push(current_category.bookmarks.pluck(:url))
    end
    
    current_category.children.each do |child|
        arr_bookmarks = child.bookmarks.pluck(:url)
        json_bookmarks = { child.name => arr_bookmarks }
        json_categories["bookmarks"][0][c_name].push( json_bookmarks)
